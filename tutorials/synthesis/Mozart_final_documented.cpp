...
// Forward Fourier Transform, width of spectrum
// Increase value for more narrow and vertically stretched spectrum
#define FFT_SIZE 4096 // Width of spectrum. 
...
public:
  ...
  // FFT_SIZE / 4: This parameter is the hop size, which specifies the number of samples to advance between successive frames. It is often a fraction of the FFT size. In this case, it is set to one-fourth of the FFT size, which means there is a 75% overlap between consecutive frames. Higher overlap provides better temporal resolution at the expense of increased computation.
  // Choose a larger denominator for less overlapping value
  // 0: This parameter is typically the pad size or the amount of zero-padding to be applied to the input signal before performing the FFT. Zero-padding can improve the frequency resolution of the FFT output. Here, it is set to zero, meaning no additional padding.
  gam::STFT stft = gam::STFT(FFT_SIZE, FFT_SIZE / 10, 0, gam::HANN, gam::MAG_FREQ);
  ...
  void init() override
  {
    ...
    mAmpEnv.curve(0); // The curve function is used to set the curvature of the envelope segments. In this context, the argument 0 specifies that the segments should be linear. This means that the transitions between different stages of the envelope (e.g., from attack to decay) will follow a straight line, resulting in a uniform rate of change.
    mAmpEnv.levels(0, 1, 1, 0); // levels(0, 1, 1, 0): The levels method sets the target amplitude levels for each stage of the envelope. In this case, the method is called with four arguments, which typically correspond to the key stages of a standard ADSR (Attack, Decay, Sustain, Release) envelope. Here's what each parameter represents:0: The initial level (start of the Attack stage). This is the starting amplitude of the envelope, usually 0 (silence).1: The peak level (end of the Attack stage and start of the Decay stage). This is the maximum amplitude reached after the attack phase. 1: The sustain level (level during the Sustain stage). This is the level during the sustain phase, which is held as long as the key or note is held down. In this case, it remains at the peak level. 0: The final level (end of the Release stage). This is the level to which the envelope decays when the note is released, usually back to 0 (silence)
  ...
  void onProcess(AudioIOData &io) override
  {
    ...
    while (io())
    {
      ...
      if (stft(s1))
      {
        ...
        {
          // Hyperbolic tan function. 1.3 strikes a balance between enhancement and distortion for signal processing
          // Values less than 1 react with 1 large spectogram that oscillates
          // Values larger than 1 reacts with multiple waves and oscillations matching the frequency of the piece
          // Values larger than 2 typically are out of range
          // higher frequency waves appear increasingly to the right, and lower frequency waves appear increasingly to the left. similar to the keys on a piano keyboard
  ...
  void onProcess(Graphics &g) override
  {
    ...
    mMelodySpect.reset();
  // Change the hue of the spectogram depending on frequency and amplitude
  // Optimal size is 10 for rainbow spectrum
  // The term frequency / 500 adjusts the hue based on the frequency, ensuring it doesn’t change too drastically with small frequency variations.
  // The term spectrum[i] * 50 adjusts the hue based on the amplitude, making amplitude variations more pronounced in the color representation.
  ...
  // 110 Yellow
    ...
    // mMelodySpect.color(1.0, 0.0, 0.0); // 1.0 for red, 0.0 for green and blue
    // mMelodySpect.vertex(i, spectrum[i], 0.0);
    // }
    
    // i = 0 to i <=0 has 1 copy in the center
    // left to right 9
    for (int i = -5; i <= 4; i++) // 1 copy in the center
    {
      g.meshColor();
      g.pushMatrix();
      ...
      // (2) distance from center, center is 0
      // (3) depth position in z-axis, more negative appears as a shrinkage parameter
      g.translate(i, -2.7, -10);

      //This kind of transformation is often used in animations or dynamic graphics where the position of an object changes over time, and frequency might be a parameter that varies to create smooth or periodic movements.
      ...
      // SCALE OBJECT
      // (1) 50.0 / FFT_SIZE: Scale factor along the x-axis. This makes the width of the object proportional to the inverse of FFT_SIZE.
      // (2) 250: Scale factor along the y-axis. This significantly stretches the object vertically.
      // (3) stretches depth of spectogram in z-axis
      g.scale(50.0 / FFT_SIZE, 250, 3.0);
      // g.pointSize(1 + 5 * mEnvFollow.value() * 10);
      
      // 1: Base line width.
      // 20 * mEnvFollow.value() * 100: Additional line width based on the value of mEnvFollow. This might be an envelope follower that tracks the amplitude of the audio signal, making the line width dynamic and responsive to audio amplitude.
      g.lineWidth(1 + 20 * mEnvFollow.value() * 100); // + 5
      g.draw(mMelodySpect);
      g.popMatrix();
    }
...
public:
  ...
  void playNote(
      ...
      float releaseTime,
      float pan,
      //float freqMult = getInternalParameterValue("speed"),
      float freqMult = 1.0)
  {
